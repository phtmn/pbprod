            ...
            <div class="card border-3 py-1 p-md-2 p-xl-3 p-xxl-4">
                <div class="card-body text-end">
                    ...
                    <form id="favorite-user-form" action="{{ route('favorites.store') }}" method="POST">
                        ...
                        <h3 class="h5 mb-2">{{ $userShow->name }} 
                            @if(auth()->check() && auth()->user()->id !== $userShow->id)
                            <button id="user-fav-submit" type="submit" class="badge btn btn-secondary text-warning">
                                <i class="@if(getUserFavorite($userShow->id)) ai-star-filled text-warning @else ai-star @endif" id="starUser"></i>
                            </button>
                            @endif
                        </h3>
                    </form>
